    ...
    for (int i = 0; i < compaction_picker_->NumberLevels() - 1; i++) {
        ...
        if (start_level_score_ >= 1) {
            ...
            if (PickFileToCompact()) {
                // 选择进行compaction的file, 当start level选中的文件不为0的时候返回true
                // found the compaction!
                if (start_level_ == 0) {
                ...
                // 注意这里有break啊，只要当前这个level有符合条件的level就选择当前level
                // 只选一个level！
                break;
            } else {
...
Compaction *LevelCompactionBuilder::GetCompaction() {
    ...
    auto c = new Compaction(
            ...
            start_level_score_, false /* deletion_compaction */, compaction_reason_, pendding_compaction_);
...
bool LevelCompactionBuilder::PickFileToCompact() {
    ...
    for (cmp_idx = vstorage_->NextCompactionIndex(start_level_);
        ...
        // 从start level的CompactionIndex开始选择input file
        ...
        // 从outputlevel选择被input key range覆盖的file，加入input中
...
Compaction *FIFOCompactionPicker::PickTTLCompaction(
    ...
    for (const auto &f : inputs[0].files) {
        ROCKS_LOG_BUFFER(log_buffer,
                         "[%s] FIFO compaction: picking file %"
                                 PRIu64
                                 " with creation time %"
                                 PRIu64
                                 " for deletion",
...
Compaction *FIFOCompactionPicker::PickSizeCompaction(
    ...
    if (total_size <=
        ...
        ROCKS_LOG_BUFFER(
                log_buffer,
                "[%s] FIFO compaction: nothing to do. Total size %"
                        PRIu64
                        ", max size %"
                        PRIu64
                        "\n",
