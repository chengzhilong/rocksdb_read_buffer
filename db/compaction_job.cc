...
const char *GetCompactionReasonString(CompactionReason compaction_reason) {
    switch (compaction_reason) {
        ...
        case CompactionReason::kNVMCacheRangeFull:
            return "NVMCacheRangeFull";
...
Status CompactionJob::InstallCompactionResults(
...

    {
        Compaction::InputLevelSummaryBuffer inputs_summary;
        ROCKS_LOG_INFO(
                db_options_.info_log, "[%s] [JOB %d] Compacted %s => %"
                PRIu64
                " bytes",
    ...
    for (const auto &sub_compact : compact_->sub_compact_states) {
        for (const auto &out : sub_compact.outputs) {
            DBG_PRINT("Generate File [%lu] size [%lu]", out.meta.fd.packed_number_and_path_id, out.meta.fd.file_size);
...
Status CompactionJob::OpenCompactionOutputFile(
    ...
    if (!s.ok()) {
        ROCKS_LOG_ERROR(
                ...
                "[%s] [JOB %d] OpenCompactionOutputFiles for table #%"
                        PRIu64
